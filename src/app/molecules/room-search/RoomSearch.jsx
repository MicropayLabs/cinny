...
function RoomSearch({ roomId }) {
	...
	const searchTerm =
		searchData?._query.search_categories.room_events.search_term ?? '';
			...
			{timeline.map((mEvent) => {
				...
				return (
					<React.Fragment key={id}>
						<Message mEvent={mEvent} isBodyOnly={false} time={time} />
				...
				{!isRoomEncrypted && searchData === null && (
					<div className="room-search__help">
						...
						{status.type === cons.status.IN_FLIGHT && (
							<Text>Searching room messages...</Text>
						)}
						{status.type === cons.status.PRE_FLIGHT && (
							<RawIcon src={SearchIC} size="large" />
						)}
						{status.type === cons.status.PRE_FLIGHT && (
							<Text>Search room messages</Text>
						)}
						{status.type === cons.status.ERROR && (
							<Text>Failed to search messages</Text>
						)}
					</div>
				)}
