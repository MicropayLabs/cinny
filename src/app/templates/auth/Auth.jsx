...
const CinnySvg = '/res/svg/cinny.svg';
...
function Homeserver({ onChange }) {
	...
	const setupHsConfig = async (servername) => {
		...
		Promise.allSettled([tempClient.loginFlows(), tempClient.register()])
			.then((values) => {
				const loginFlow =
					values[0].status === 'fulfilled' ? values[0]?.value : undefined;
				const registerFlow =
	...
	useEffect(async () => {
		...
		const configFileUrl = `${link}${
			link[link.length - 1] === '/' ? '' : '/'
		}config.json`;
		try {
			const result = await (
				await fetch(configFileUrl, { method: 'GET' })
			).json();
		...
		<>
			<div className="homeserver-form">
				...
				<ContextMenu
					...
					render={(toggleMenu) => (
						<IconButton onClick={toggleMenu} src={ChevronBottomIC} />
					)}
				/>
			</div>
...
function Login({ loginFlow, baseUrl }) {
	...
	const isPassword = loginFlow?.filter(
		(flow) => flow.type === 'm.login.password'
	)[0];
	const ssoProviders = loginFlow?.filter(
	...
	const validator = (values) => {
		...
		if (
			...
			!isValidInput(values.email, EMAIL_REGEX)
		) {
...
function Register({ registerInfo, loginFlow, baseUrl }) {
	...
	flows?.forEach((flow) => {
		...
		if (!isRecaptcha)
			isRecaptcha = flow.stages.indexOf('m.login.recaptcha') > -1;
	...
	const validator = (values) => {
		...
		if (
			values.username.length > 0 &&
			!isValidInput(values.username, LOCALPART_SIGNUP_REGEX)
		) {
			errors.username = BAD_LOCALPART_ERROR;
		}
		if (
			values.password.length > 0 &&
			!isValidInput(values.password, PASSWORD_STRENGHT_REGEX)
		) {
	...
	useEffect(() => {
		...
		const asyncProcess = async () => {
			...
			if (isDummy) {
				const data = await auth.completeRegisterStage(
					baseUrl,
					username,
					password,
					{
	...
	return (
		<>
			...
			{process.type === 'm.login.terms' && (
				<Terms url={process.url} onSubmit={handleTerms} />
			)}
			...
			<div className="auth-form__heading">
				...
				{isDisabled && (
					<Text className="auth-form__error">{registerInfo.error}</Text>
				)}
			</div>
			{!isDisabled && (
				<Formik
		...
		<ProcessWrapper>
			...
			>
				<div style={{ margin: 'var(--sp-normal)', maxWidth: '450px' }}>
					...
					<div style={{ marginBottom: 'var(--sp-normal)' }} />
					<Text variant="b1">
						In order to complete registration, you need to agree to the terms
						and conditions.
					</Text>
					<div
						style={{
